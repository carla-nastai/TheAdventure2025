...
namespace TheAdventure
{
    public class GameLogic
    {
        ...
        private readonly Dictionary<int, Tile> _tileIdMap = new Dictionary<int, Tile>();
        private Level _currentLevel = new Level();

        private int _bombIds = 100;

        public void InitializeGame(Sdl sdl, GameWindow gameWindow)
        {
            // Ensure GameRenderer is initialized before loading textures
            var gameRenderer = GameRenderer.GetInstance(sdl, gameWindow, this);
       ...
       public void RunGameLoop(Sdl sdl, GameWindow gameWindow)
{
    // Ensure that you pass the already created window to the GameRenderer
    var gameRenderer = GameRenderer.GetInstance(sdl, gameWindow, this);

    var inputLogic = new InputLogic(sdl, this);
...
public void StartGame()
{
    ...
    {
        throw new Exception($"SDL initialization failed: {sdl.GetErrorS()}");
    }

    // Get the singleton instance of GameWindow
    var gameWindow = GameWindow.GetInstance(sdl);

    // Initialize the game
    InitializeGame(sdl, gameWindow);

    // Run the game loop
    RunGameLoop(sdl, gameWindow);
}
